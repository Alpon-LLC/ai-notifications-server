...
- `PR_REVIEW_SLACK_CHANNEL_ID`: optional; when set, a "PR Review Started" Slack notification is posted to this channel AFTER the review payload is successfully forwarded to the review agent (action `opened` only). Absent = skipped (debug log).
